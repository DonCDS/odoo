...
class MergePartnerAutomatic(models.TransientModel):
    ...
    def _update_reference_fields(self, src_partners, dst_partner):
        ...
        for record in record_data:
            try:
                ...
                field_type = proxy_model._fields[record.name].type
            except KeyError:
    ...
    def _merge(self, partner_ids, dst_partner=None):
        ...
        if self.sudo()._uid != self._uid and len(set(partner.email for partner in partners)) > 1:
            raise UserError(_("All contacts must have the same email. Only the Administrator can merge contacts with different emails."))

        if dst_partner and dst_partner.id in partner_ids:
            src_partners_ids = partner_ids
            src_partners_ids.remove(dst_partner.id)
            src_partners = self.env['res.partner'].browse(src_partners_ids)
        else:
        ...
        if self.sudo()._uid != self._uid and self._model_is_installed('account.move.line') and \
    ...
    def next_cb(self):
        ...
        return self.with_context(active_test=False)._next_screen()[0]
    ...
    def _model_is_installed(self, model):
        domain = [('model', '=', model)]
        return self.env['ir.model'].search_count(domain) > 0
    ...
    def auto_set_parent_id(self):
        ...
        for id, email in self._cr.fetchall():
            ...
            self._cr.execute("""  SELECT id,email
                            FROM res_partner
