...
class crm_lead(format_address, osv.osv):
    ...
    def _track_subtype(self, cr, uid, record, values, context=None):
        if 'stage_id' in values and record.probability == 100 and record.stage_id and record.stage_id.fold:
            return 'crm.mt_lead_won'
        elif 'stage_id' in values and record.probability == 0 and record.stage_id and record.stage_id.fold and record.stage_id.sequence > 1:
            return 'crm.mt_lead_lost'
        elif 'stage_id' in values and record.probability == 0 and record.stage_id and record.stage_id.sequence <= 1:
            return 'crm.mt_lead_create'
        elif 'stage_id' in values:
            return 'crm.mt_lead_stage'
        return super(crm_lead, self)._track_subtype(cr, uid, record, values, context=context)
