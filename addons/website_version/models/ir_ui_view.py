...
class view(models.Model):
    ...
    def translate_qweb(self, id_, arch, lang):
        ...
        views = self.search([('key', '=', view.key), '|', ('website_id', '=', view.website_id.id), ('website_id', '=', False)])
        fallback_views = self
        for v in views:
        ...
        def translate_func(term):
            ...
            trans = Translations._get_source('website', 'view', lang, term, tuple(views.ids))
            return trans
    ...
    def get_view_translations(self, xml_id, lang, field=['id', 'res_id', 'value', 'state', 'gengo_translation']):
        ...
        view_list = self.search([('key', '=', view.key), '|', ('website_id', '=', view.website_id.id), ('website_id', '=', False)])
        views_ids = []
        for v in view_list:
            views = self.customize_template_get(v.id, full=True)
            views_ids += [view.get('id') for view in views if view.get('active')]
        domain = [('type', '=', 'view'), ('res_id', 'in', views_ids), ('lang', '=', lang)]
        element_list = self.pool['ir.translation'].search_read(self.env.cr, self.env.uid, domain, field, context=self.env.context)
