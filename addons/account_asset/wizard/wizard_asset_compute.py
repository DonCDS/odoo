...
class asset_depreciation_confirmation_wizard(models.TransientModel):
    ...
    period_id = fields.Many2one('account.period', string='Period', default=_get_period, required=True, help="Choose the period for which you want to automatically post the depreciation lines of running assets")

    @api.multi
    def asset_compute(self):
        assets = self.env['account.asset.asset'].search([('state', '=', 'open'), ('category_id.type', '=', self._context.get('type'))])
