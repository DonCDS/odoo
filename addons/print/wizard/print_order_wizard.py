...
class print_order_wizard(models.TransientModel):
    ...
    provider_environment = fields.Selection([('test', 'Test'), ('production', 'Production')], "Environment", default=False)
    ...
    def action_apply(self):
        ...
        for wizard in self:
            ...
            if wizard.provider_balance <= 0.0 and wizard.provider_environment == 'production':
                raise Warning(_('Your credit provider is too small. Please, configure your account (> Settings > Print Provider), and put some credit on your account, since you are in Production Mode.'))
...
class print_order_line_wizard(models.TransientModel):
    ...
    def _default_selection_state(self):
        return [('ok', 'Ready to be sent'), ('bad_postal_address', 'Wrong Postal Address'), ('bad_invoice_state', ('Wrong Invoice State'))]

    @api.one
    @api.depends('partner_id', 'res_model')
    def _compute_state(self):
        state = 'ok'
        if not self.partner_id.is_address_valid:
            state = 'bad_postal_address'
        if self.res_model == 'account.invoice':
            if self.env['account.invoice'].browse(self.res_id).state != 'open':
                state = 'bad_invoice_state'
        self.state = state
