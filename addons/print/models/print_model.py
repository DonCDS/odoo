...
class print_order(models.Model):
    ...
    send_date = fields.Datetime('Sending Date', readonly=True)
    ...
    def _prepare_printing(self):
        """ Prepare the orders for delivery. It executes the operations to put
            them into the 'ready_to_send' state (or 'error' if something wrong happens).
    ...
    def _deliver_printing(self):
        """ Send the orders for delivery to the Provider. It executes the operations to put
            ...
            Required : the print.order must be in the 'ready_to_send' state.
    ...
    def _validate_printing(self):
        ...
        """
        # group the PO by res_model (res_model is optional field, so only keep PO where res_model not None/False)
        res_models = set(self.filtered(lambda r: r.res_model).mapped('res_model'))
    ...
    def process_order_queue(self, order_ids=None):
        ...
        # deliver only the 'ready_to_send' ones
        orders.filtered(lambda r: r.state == 'ready_to_send')._deliver_printing()
        ...
        user_to_notify = {}
        for record in orders.filtered(lambda rec: rec.state == 'error'):
        ...
        for user_id in user_to_notify.keys():
            template.with_context(print_errors=user_to_notify[user_id]).sudo().send_mail(user_id, force_send=True)
...
class print_mixin(models.AbstractModel):
