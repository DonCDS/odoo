...
class hr_holidays(osv.osv):
    ...
    def _track_subtype(self, cr, uid, record, values, context=None):
        if 'state' in values and record.state == 'validate':
            return 'hr_holidays.mt_holidays_approved'
        elif 'state' in values and record.state == 'validate1':
        ...
        elif 'state' in values and record.state == 'refuse':
            return 'hr_holidays.mt_holidays_refused'
        return super(hr_holidays, self)._track_subtype(cr, uid, record, values, context=context)
