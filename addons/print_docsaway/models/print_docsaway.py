...
class DocsawayException(Exception):
    pass
...
class print_provider(models.Model):
    ...
    def docsaway_check_configuration(self):
        ...
        if not self.docsaway_email and not self.docsaway_key:
            raise Warning(_('You must configure your Docsaway Account : the Docsaway Email and Docsaway Key are required. You will find them on www.docsaway.com'))
    ...
    def _docsaway_get_credentials(self):
        ...
        if not (self.docsaway_email and self.docsaway_key):
            raise Warning(_("Please configure your DocsAway Print Provider Account. See in Settings > Print Provider. More information on www.docsaway.com ."))
    ...
    def _docsaway_do_server_request(self, url, request, error_map=_DOCSAWAY_ERROR_MAIL):
        ...
        if response['APIErrorNumber']:
            ...
            raise DocsawayException("DOCSAWAY ERROR : " + error_code + " " + error_map.get(error_code, _('Unknown Error')))
        return response
...
class print_order(models.Model):
    ...
    def _docsaway_prepare_printing(self):
        ...
        for order in self:
            ...
            except (DocsawayException, Warning) as e:
                order.write({
                    'state' : 'error',
                    'error_message' : e,
                })
