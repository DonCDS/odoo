...
class hr_expense_report(models.Model):
        ...
        cr.execute("""
            create or replace view hr_expense_report as (
                 select
                     ...
                     count(*) as nbr_lines,
                     (select unit_quantity from hr_expense_expense where id=l.id and product_id is not null) as no_of_products,
                     (select analytic_account from hr_expense_expense where id=l.id and analytic_account is not null) as no_of_account,
                     s.state
                 from hr_expense_expense l
