...
class project_issue(osv.Model):
    ...
    def _track_subtype(self, cr, uid, record, values, context=None):
        if 'kanban_state' in values and record.kanban_state == 'blocked':
            return 'project_issue.mt_issue_blocked'
        elif 'kanban_state' in values and record.kanban_state == 'done':
            return 'project_issue.mt_issue_ready'
        elif 'user_id' in values and record.user_id:  # assigned -> new
            return 'project_issue.mt_issue_new'
        elif 'stage_id' in values and record.stage_id and record.stage_id.sequence <= 1:  # start stage -> new
            return 'project_issue.mt_issue_new'
        elif 'stage_id' in values:
            return 'project_issue.mt_issue_stage'
        return super(project_issue, self)._track_subtype(cr, uid, record, values, context=context)
