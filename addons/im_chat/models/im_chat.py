...
class im_chat_conversation_state(models.Model):
    """ User Session State
...
class im_chat_session(models.Model):
    ...
    def session_user_image(self, uuid, user_id):
        ...
        if user_id:
            ...
            if session and session.sudo(user_id).is_in_session():
                image_b64 = self.env["res.users"].browse(user_id).image_small
    ...
    @api.model
    def update_state(self, uuid, status=None):
        ...
        for session_state in self.env['im_chat.conversation_state'].search(domain):
            if not status:
                ...
                if session_state.state == 'open':
                    ...
                    status = 'open'
            session_state.write({'state': status})
...
class im_chat_message(models.Model):
    ...
    def fetch_message(self, uuid, last_id=False, limit=20):
        """ Return messages for the given session uuid. If the session is public, fetch_message
            ...
            :rtype : list(dict)
        """
        # TODO check if private (maybe in livechat) and update doc
        session = self.env['im_chat.session'].search([('uuid', '=', uuid)], limit=1)
...
class im_chat_shortcode(models.Model):
    """ Shortcode
...
class im_chat_presence(models.Model):
    ...
    def update(self, user_presence=True):
        ...
        self._cr.commit() # TODO : check if still necessary
