...
class Controller(openerp.addons.bus.bus.Controller):
    ...
    def _poll(self, dbname, channels, last, options):
        if request.session.uid:
            request.env['im_chat.presence'].update(options.get('im_presence', False))
    ...
    # --------------------------
    # Only Logged User Methods
    # --------------------------
    @openerp.http.route('/im_chat/init', type="json", auth="user") #TODO test with auth='user' instead of 'none'
    def init(self):
