...
class im_livechat_channel(models.Model):
    ...
    def get_channel_session(self, channel_id, anonymous_name):
        ...
        users = self.browse(channel_id).get_available_users()
        if len(users) == 0:
            return False
        user_id = random.choice(users).id
        # create the session, and add the link with the given channel
        session = self.env["im_chat.session"].create({'user_ids': [(4, user_id)], 'channel_id': channel_id, 'anonymous_name' : anonymous_name})
...
class im_chat_session(models.Model):
    """ Chat Session
    ...
    def is_in_session(self):
        ...
        """
        self.ensure_one()
        if self.anonymous_name and self._uid == openerp.SUPERUSER_ID: # TODO test or find better !
            return True
        else:
            return super(im_chat_session, self).is_in_session()
    ...
    def session_user_info(self):
        ...
        """
        self.ensure_one()
        users_infos = super(im_chat_session, self).session_user_info()
    ...
    def quit_user(self, uuid):
        """ Remove the current user from the given session.
            ...
            Required to modify the base comprtement, since a session can contain only 1 identified user.
        ...
        if session.anonymous_name:
            ...
            return super(im_chat_session, self).quit_user(uuid)
