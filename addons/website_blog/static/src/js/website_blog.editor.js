    ...
    openerp.website.if_dom_contains('.website_blog', function() {
            ...
            save : function () {
                ...
                if ($('#js_blogcover').length) {
                    openerp.jsonRpc("/blog/post_change_background", 'call', {
                        ...
                        'cover_properties' : '{"background-image": "' + $('#js_blogcover').css("background-image").replace(/"/g, '') + '", "background-color": "' + $('#js_blogcover').attr("class") + '", "opacity":"' + $('#js_blogcover').css("opacity")  + '", "resize_class": "' + $('#title').attr('class') + '"}',
                    }).always(function() {
        ...
        website.snippet.options.website_blog = website.snippet.Option.extend({
            start : function(type, value, $li) {
                this._super();
                $('li[data-change].snippet-option-website_blog').parents().find("a.btn.btn-primary.btn-sm").html(_t('Customize Cover ') + "<span class='caret'/>");
            ...
            clear : function(type, value, $li) {
                ...
                this.src = null;
                this.$target.find('#js_blogcover').css({"background-image": '', "opacity": "1.0"}).attr("class", "o_none");
