...
class website_event(http.Controller):
    ...
    def events(self, page=1, **searches):
        ...
        for date in dates:
            if date[0] != 'old':
                date[3] = event_obj.search_count(dom_without('date') + date[2])

        domain = dom_without('type')
        types = event_obj.read_group(domain, ["id", "type"], groupby="type", orderby="type")
        type_count = event_obj.search_count(domain)
        types.insert(0, {
        ...
        country_id_count = event_obj.search_count(domain)
        countries.insert(0, {
        ...
        event_count = event_obj.search_count(dom_without("none"))
        ...
        events_ids = event_obj.search(dom_without("none"), limit=step, offset=pager['offset'], order=order)
    ...
    def _add_event(self, event_name=None, context={}, **kwargs):
        ...
        event = Event.with_context(context).create(vals)
    ...
    def get_country_events(self, **post):
        ...
        if country_code:
            ...
            event_ids = event_obj.search(['|', ('address_id', '=', None), ('country_id.code', '=', country_code), ('date_begin', '>=', time.strftime('%Y-%m-%d 00:00:00')), ('state', '=', 'confirm')], order="date_begin")
        if not event_ids:
            event_ids = event_obj.search([('date_begin', '>=', time.strftime('%Y-%m-%d 00:00:00')), ('state', '=', 'confirm')], order="date_begin")
        for event in event_ids:
            if country_code and event.country_id.code == country_code:
                result['country'] = country_obj.browse(country_ids[0].id)
    ...
    def registration_confirm(self, event, **post):
        ...
        attendees = Registration.browse(registration_ids)
