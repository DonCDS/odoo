...
class purchase_order(osv.osv):
    ...
    def _track_subtype(self, cr, uid, record, values, context=None):
        if 'state' in values and record.state == 'approved':
            return 'purchase.mt_rfq_approved'
        elif 'state' in values and record.state == 'confirmed':
            return 'purchase.mt_rfq_confirmed'
        elif 'state' in values and record.state == 'done':
            return 'purchase.mt_rfq_done'
        return super(purchase_order, self)._track_subtype(cr, uid, record, values, context=context)
