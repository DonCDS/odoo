...
class Field(object):
    ...
    def _setup_regular(self, env):
        ...
        def make_callable(method):
            if isinstance(method, basestring):
                return lambda recs: getattr(recs, method)()
            return method
    ...
    def _setup_related(self, env):
        ...
        if not (self.readonly or field.readonly):
            self._inverse = self._inverse_related
        if field._description_searchable:
            # allow searching on self only if the related field is searchable
            self._search = self._search_related
    ...
    def get_inverse_fields(self, env=None):
        reg = registry() if env is None else env.registry
        return reg.field_inverses.get(self, [])

    def add_inverse_field(self, env, field):
        inverse_fields = env.registry.field_inverses.setdefault(self, [])
    ...
    def add_triggers(self, env, triggers):
        field_triggers = env.registry.field_triggers.setdefault(self, set())
        field_triggers.update(triggers)
...
class Float(Field):
    ...
    def _setup_digits(self, env):
        ...
        digits = self._digits(env.cr) if callable(self._digits) else self._digits
        if digits:
            assert isinstance(digits, (tuple, list)) and len(digits) >= 2, \
                "Float field %s with digits %r, expecting (total, decimal)" % (self, digits)
        env.registry.field_digits[self] = digits
...
class _Relational(Field):
    ...
    def modified(self, records):
        # Invalidate cache for inverse fields, too. Note that recomputation of
        # fields that depend on inverse fields is already covered by the
        # triggers (see above).
...
class _RelationalMulti(_Relational):
    ...
    def convert_to_write(self, value, target=None, fnames=None):
        ...
        if fnames is None:
            ...
            for invf in self.get_inverse_fields():
